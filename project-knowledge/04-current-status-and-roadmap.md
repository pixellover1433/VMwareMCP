...
- **Power tools** — [`get_vm_power_state()`](src/vmware_mcp/modules/power.py:29) and [`set_vm_power_state()`](src/vmware_mcp/modules/power.py:56) supporting all 8 power operations.
- **Snapshot tools removed** — Snapshot MCP tools were intentionally removed from [`modules/snapshot.py`](src/vmware_mcp/modules/snapshot.py). The client methods still exist in [`vmrest_client.py`](src/vmware_mcp/vmrest_client.py:179) but are not exposed as MCP tools.
...
1. **Snapshot module is empty** — [`modules/snapshot.py`](src/vmware_mcp/modules/snapshot.py) is a placeholder. The snapshot client methods in [`vmrest_client.py`](src/vmware_mcp/vmrest_client.py:179) (`get_snapshots`, `create_snapshot`, `delete_snapshot`, `revert_snapshot`) are implemented but not wired to MCP tools.
2. **No tests** — The project has no unit or integration tests. The `vmrest_client.py` and module logic are untested.
3. **No error handling for missing displayName** — [`get_vms()`](src/vmware_mcp/vmrest_client.py:87) calls [`get_vm_param()`](src/vmware_mcp/vmrest_client.py:68) for each VM to get `displayName`, resulting in N+1 API calls. This could be slow for hosts with many VMs.
...
7. **No graceful shutdown** — The server does not call [`VMRestClient.close()`](src/vmware_mcp/vmrest_client.py:233) on exit.
...
- [ ] Add retry logic with exponential backoff to [`VMRestClient._request()`](src/vmware_mcp/vmrest_client.py:42).
